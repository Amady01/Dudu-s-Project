# Fiche équipe — GET409-NiayesBiz

---

## Identité de l'équipe

| Champ | Détail |
|---|---|
| Nom de l'équipe | Dudu-s-Project |
| Secteur cible | mecanique |
| Projet fil rouge | mecanique —  |
| Dépôt GitHub | https://github.com/Amady01/Dudu-s-Project |
| Workspace Dify | Dudu-s-Project |

---

## Membres & rôles

| Prénom Nom | Rôle | GitHub |
|---|---|---|
| Djiby DIALLO | Chef de Produit (PM) |@Djiby20 |
| Doudou SARR | Master Prompt Engineer | @du-dev|
| Amadou SALL | Dev UI (No-Code) | @Amady01 |
| Ramatoulaye SY | Responsable Impact | @|
| Alassane DIAW | Pentesteur | @lazou4596|
| Jordan Sandrick| Testeur | @Jordan-Sandrick|

---

## Notre défi

**Secteur :** Mécanique — Projet ---> AutoForge

**Problème observé :**
 Les propriétaires de véhicules ont beaucoup de difficultés pour trouver des piéces de rechanges de leurs véhicules

**Énoncé HMW :**
> "Comment pourrions-nous aider les propriétaires de véhicules à comparer en temps réel la disponibilité et le prix des pièces détachées chez les revendeurs locaux, afin de réduire le temps d'immobilisation de leur voiture et d'éviter les surcoûts ?"


---

## Infrastructure S1

- [x] Dépôt GitHub public créé — `Dudu-s-Project`
- [x] README.md initialisé et mis à jour
- [x] Dossier `docs/` créé
- [x] Workspace Dify configuré — `AutoForge`
- [ ] Tous les membres invités (rôle Write)

---

## Répartition des tâches S1

| Tâche | Responsable | Statut |
|---|---|---|
| Création dépôt GitHub | Dev UI | ✅ |
| Fiche équipe Markdown | PM | ✅ |
| Interview d'empathie | Tous | ✅ |
| Carte d'empathie Markdown | Prompt Engineer | ✅ |
| Énoncé HMW | PM + Impact | ✅ |
| README mis à jour | Dev UI | ✅ |
